...
class TrainEntry:
	def __init__(self, label, tensor, number):
		self.label = label
		self.tensor = tensor
		self.number = number
...
def train_model(entries):
	model = RNN(input_size, hidden_size, categories)
	...
	with torch.no_grad():
		for entry in entries:
			...
			if (predicted == label):
				...
				print('Label: ' + body_labels[label] + ' - ' + str(entry.number) + ", predicted: " + body_labels[predicted])
			if (predicted == label):
